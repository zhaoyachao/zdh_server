...
//        statement.close()
  ...
  def insertQuality(task_log_id:String,dispatch_task_id:String,job_context:String,etl_task_id:String,etl_context:String,etl_date:String,report:Map[String,String],owner:String): Unit ={
    ...
    try {
      ...
      //ETL_DATE,MODEL_NAME,STATUS,START_TIME,END_TIME
      val sql = s"insert into quality(id,dispatch_task_id,etl_task_id,etl_date,status,report,create_time,owner,etl_context,job_context) values(?,?,?,?,?,?,?,?,?,?)"
      val statement = connection.prepareStatement(sql)
      ...
      statement.setString(9, etl_context)
      statement.setString(10, job_context)
      statement.execute()
      statement.close()
  ...
  def getConnect(): Boolean = {
    ...
    val inStream = this.getClass.getResourceAsStream("/datasources.properties")
    props.load(inStream)
    if(props.getProperty("enable").equals("false")){
      logger.info("读取配置文件datasources.properties,但是发现未启用此配置enable is false")
      return false
    }
