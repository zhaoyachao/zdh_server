...
import com.zyc.zdh.datasources.sftp.util.Utils.ImplicitDataFrameWriter
 ...
 */
class SftpSource extends RelationProvider with SchemaRelationProvider with CreatableRelationProvider  {
  @transient val logger = Logger.getLogger(classOf[SftpSource])
  ...
  override def createRelation(sqlContext: SQLContext, parameters: Map[String, String], schema: StructType) = {
    ...
    val supportedFileTypes = List("csv", "json", "avro", "parquet", "txt", "xml","orc","excel")
    ...
    if (!createDF.toBoolean) {
      ...
      SftpRelation(fileLocation, fileType, inferSchemaFlag, header, delimiter, quote, escape, multiLine, rowTag, schema,parameters,
        sqlContext)
    }
  }
