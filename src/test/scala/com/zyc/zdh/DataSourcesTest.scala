...
@Test
class DataSourcesTest extends TEST_TRAIT2{
  @Test
  def testDataHandlerJDBC {
    ...
    val opt=Map(
      ...
      "url"->"jdbc:mysql://127.0.0.1:3306/zdh_test?serverTimezone=GMT%2B8",
      "dbtable"->"z1",
      "user"->"zyc",
      "password"->"123456"
    )
    ...
    DataSources.DataHandler(spark,null,Map.empty[String,Any],null,input,opt,"1=1",null,output,outputOptions,cols,sql)


  }

    @Test
  def testDataHandlerJDBC2Hive {
...
//    df.show()
  ...
  def testRegex {
    var a = "^1(3|4|5|6|7|8|9)\\d{9}$".r()
    println("152364798060".matches(a.toString()))


    import spark.implicits._
    val user_seq = Seq(("15236479806", "001", "p1", 1),
      ("20200325", "001", "p1", 1),
      ("20200325", "001", "p1", 1),
      ("20200325", "001", "p1", 1),
      ("20200325", "001", "p1", 1),
    ...
    val ds = user_seq.toDF("phone", "user_id", "page_id", "time")

    ds.selectExpr("count(if(phone rlike '"+"1(3|4|5|6|7|8|9)\\\\d{9}"+"', 1,null))").show()
